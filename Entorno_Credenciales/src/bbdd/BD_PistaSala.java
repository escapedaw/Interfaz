...
public class BD_PistaSala extends BD_Conector{
	...
	/**
	 * Metodo que muestra las identificaciones de los empleados disponibles
	 * @return vector identificaciones
	 * @throws TecnicaExcepcion 
	 */
	
	public  Vector<String> mostrarIdentEmple (){ 
		String cadenaSQL="SELECT ID from empleados";
		Vector<String> identEmple=new Vector<String>();
		try{
			this.abrir();
			s=c.createStatement();
			reg=s.executeQuery(cadenaSQL);
			while ( reg.next()){
				identEmple.add(reg.getString("ID"));
				// necesito el constructor (idEmple)
			}
			s.close();
			this.cerrar();
			return identEmple;
		...
		/*catch (ClassNotFoundException e) { // POR QUE
			throw new TecnicaExcepcion("En este momento no podemos acceder a la base de datos");
		}*/		
	}
	
	/**
	 * Metodo que muestra las identificaciones de los jefes disponibles
	 * @return vector identificaciones
	 * @throws TecnicaExcepcion 
	 */
	
	public  Vector<String> mostrarIdentJefe () { 
		String cadenaSQL="SELECT ID from jefes";
		Vector<String> identJefe=new Vector<String>();
		try{
			this.abrir();
			s=c.createStatement();
			reg=s.executeQuery(cadenaSQL);
			while ( reg.next()){
				identJefe.add(reg.getString("ID"));
			}
			s.close();
			this.cerrar();
			return identJefe;
